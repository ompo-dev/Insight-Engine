...
export function WorkspaceInlineSwitcher({
  ...
  useLayoutEffect(() => {
    ...
    const itemNodes = Array.from(
      bodyInner.querySelectorAll("[data-switcher-item]"),
    );
    const closedHeight = Math.max(collapsedHeight, header.offsetHeight);
    const targetHeaderHeight =
      open && collapseTriggerWhenOpen ? 0 : closedHeight;
  ...
  const hasMeta = Boolean(
    title || subtitle || badge || (actionLabel && onAction),
  );
    ...
    <div ref={rootRef} className="relative inline-flex shrink-0">
      ...
      >
        <button
          ...
          onClick={onToggle}
          className={cn(
            "relative z-10 flex w-full shrink-0 overflow-hidden items-center text-left",
            headerClassName,
          )}
        >
          {trigger}
        ...
        >
          ...
          >
            ...
            <div
              className={cn("max-h-[320px] overflow-y-auto", hasMeta && "mt-4")}
            >
              {items.map((item) => (
                <button
                  key={item.id}
                ...
                >
                  ...
                  <ChevronDown
                    className={cn(
                      "h-4 w-4 shrink-0 rotate-[-90deg] text-white/35",
                      item.active && "text-white/68",
                    )}
                  />
                </button>
              ))}
