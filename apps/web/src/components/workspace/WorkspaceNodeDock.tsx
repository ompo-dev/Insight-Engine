...
export function WorkspaceNodeDock({
  ...
  const [dockLayout, setDockLayout] = useState({ width: 820, bottom: 24 });

  useLayoutEffect(() => {
    const measure = () => {
      const viewportWidth = window.innerWidth;
      const baseBottom = viewportWidth < 768 ? 16 : 24;
      const baseWidth = Math.max(320, Math.min(820, viewportWidth - 32));
      const leftFooter = document.querySelector(
        "[data-workspace-footer-left='true']",
      ) as HTMLElement | null;
      const rightFooter = document.querySelector(
      ...
      if (leftFooter && rightFooter) {
        const leftRect = leftFooter.getBoundingClientRect();
        const rightRect = rightFooter.getBoundingClientRect();
        const centerGap = Math.floor(rightRect.left - leftRect.right - 32);

        if (centerGap >= 320) {
          nextWidth = Math.min(baseWidth, centerGap);
        } else {
          const footerTop = Math.min(leftRect.top, rightRect.top);
          nextBottom = Math.max(
            baseBottom,
            Math.ceil(window.innerHeight - footerTop + 16),
          );
        }
      }

      setDockLayout((current) =>
        current.width === nextWidth && current.bottom === nextBottom
          ? current
          : { width: nextWidth, bottom: nextBottom },
      );
    };
    ...
    if (observer && leftFooter) observer.observe(leftFooter);
    if (observer && rightFooter) observer.observe(rightFooter);
  ...
  useLayoutEffect(() => {
    ...
    const expandedHeight = Math.max(
      collapsedHeight,
      Math.min(window.innerHeight - dockLayout.bottom - 24, 748),
    );
  ...
  }, [dockLayout.bottom, expanded, node?.id]);
  ...
  return (
    <form
      ...
      className={cn(
        "workspace-node-dock-shell cq-panel pointer-events-auto absolute left-1/2 z-30 flex h-fit -translate-x-1/2 flex-col overflow-hidden rounded-[34px] border border-white/10 bg-[#16181d]/95 p-4 text-white shadow-[0_34px_82px_rgba(0,0,0,0.46)] backdrop-blur-2xl",
      )}
      style={{ width: dockLayout.width, bottom: dockLayout.bottom }}
    >
      <div
        ref={composerRef}
      ...
      >
        ...
        />
        <div className="workspace-node-dock-actions flex flex-wrap items-center justify-between gap-3">
          <div className="workspace-node-dock-chip-group flex items-center gap-2">
            <DockChip
          ...
          <div className="workspace-node-dock-chip-group flex items-center gap-2">
            <button
              type="submit"
              className="workspace-node-dock-submit inline-flex h-10 items-center justify-center rounded-full border border-sky-300/20 bg-sky-400/12 px-4 text-sm font-semibold text-sky-100 transition hover:bg-sky-400/18"
            >
              Enviar
            </button>
          </div>
        ...
        {node && presentation ? (
          <div className="flex h-full min-h-0 flex-col gap-4">
            <div ref={previewRef} className="opacity-0">
              <div className="workspace-node-dock-focus flex items-start justify-between gap-4 rounded-[28px] border border-white/8 bg-[#10141a]/82 px-4 py-4">
                ...
                <div className="workspace-node-dock-focus-actions flex items-center gap-3">
...
function DockChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "workspace-node-dock-chip inline-flex h-10 items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3.5 text-sm font-medium text-slate-200 transition hover:bg-white/8",
        className,
      )}
    >
      <span className="workspace-node-dock-chip__icon text-white/70">{icon}</span>
      <span className="workspace-node-dock-chip__label">{label}</span>
    </button>
  );
}
