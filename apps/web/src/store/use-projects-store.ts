...
interface ProjectsStore {
  ...
  hydrated: boolean;
  prime: (input: { projects?: ProjectSummary[]; settings?: Record<string, ProjectSettings> }) => void;
...
export const useProjectsStore = create<ProjectsStore>((set, get) => ({
  ...
  isSettingsLoading: {},
  hydrated: false,
  prime: ({ projects, settings }) => {
    set((state) => ({
      projects: projects ?? state.projects,
      settingsByProject: settings ? { ...state.settingsByProject, ...settings } : state.settingsByProject,
      hydrated: (projects?.length ?? state.projects.length) > 0 || state.hydrated,
    }));
  },
  hydrate: async () => {
