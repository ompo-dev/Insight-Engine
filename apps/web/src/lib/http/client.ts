...
import { useMocksEnabled } from "@/lib/env/public";

const USE_MOCKS = useMocksEnabled();
...
async function apiFirstOrMock<T>(apiFactory: () => Promise<T>, mockFactory: () => T): Promise<T> {
  try {
    return await apiFactory();
  } catch {
    return wait(mockFactory);
  }
}
