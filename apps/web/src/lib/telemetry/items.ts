...
export type SpecialTelemetryNodeKind =
  | "terminal"
  | "markdown"
  | "ai"
  | "file-manager"
  | "file-viewer"
  | "browser";
export type TerminalNodeShell = "bash" | "zsh" | "powershell" | "cmd";
...
export interface FileManagerNodeConfig {
  assetIds: string[];
...
export interface FileViewerNodeConfig {
  assetId?: string | null;
...
export interface CustomTelemetryItemDefinition {
  ...
  ai?: AiNodeConfig;
  fileManager?: FileManagerNodeConfig;
  fileViewer?: FileViewerNodeConfig;
  browser?: BrowserNodeConfig;
...
export interface CreateCustomTelemetryItemInput {
  ...
  fileManager?: Partial<FileManagerNodeConfig>;
  fileViewer?: Partial<FileViewerNodeConfig>;
  browser?: Partial<BrowserNodeConfig>;
}
...
function defaultTerminalNodeConfig(): TerminalNodeConfig {
  return {
    ...
    liveOutput: [
      "Microsoft Windows [versao local]",
      ".>",
...
function defaultFileManagerNodeConfig(): FileManagerNodeConfig {
  return {
    assetIds: [],
    sortBy: "recent",
    filter: "",
...
function defaultFileViewerNodeConfig(): FileViewerNodeConfig {
  return {
    assetId: null,
    viewerType: "document",
    previewState: "ready",
...
function defaultBrowserNodeConfig(): BrowserNodeConfig {
  return {
...
function buildSpecialNodePayload(kind: SpecialTelemetryNodeKind, slug: string): Record<string, unknown> {
  ...
  if (kind === "browser") {
    return {
      url: "https://example.com",
      title: "Browser Node",
      text: "Conteudo da pagina sera materializado aqui.",
...
function resolveSpecialNodeDefaults(kind: SpecialTelemetryNodeKind | undefined, slug: string) {
  ...
  return {
    ...
    fileManager: kind === "file-manager" ? defaultFileManagerNodeConfig() : undefined,
    fileViewer: kind === "file-viewer" ? defaultFileViewerNodeConfig() : undefined,
    browser: kind === "browser" ? defaultBrowserNodeConfig() : undefined,
  };
}
...
export function createCustomTelemetryItemDraft(args: {
  ...
  return {
    ...
    ai: specialKind === "ai"
      ...
      : undefined,
    fileManager: specialKind === "file-manager"
      ? { ...defaultFileManagerNodeConfig(), ...args.seed?.fileManager }
      : undefined,
    fileViewer: specialKind === "file-viewer"
      ? { ...defaultFileViewerNodeConfig(), ...args.seed?.fileViewer }
      : undefined,
    browser: specialKind === "browser"
      ? { ...defaultBrowserNodeConfig(), ...args.seed?.browser }
      : undefined,
    executionRuns: [],
    actionDeliveries: [],
    createdAt: now,
...
function buildSpecialNodePreview(item: CustomTelemetryItemDefinition): TelemetryExpressionPreview | null {
  ...
  if (item.specialKind === "file-manager") {
    const total = item.fileManager?.assetIds.length ?? 0;
    const text = total ? `${total} arquivo(s) prontos` : "Nenhum arquivo carregado";
    return {
      kind: "text",
      raw: { assetIds: item.fileManager?.assetIds ?? [] },
      text,
    };
  }

  if (item.specialKind === "file-viewer") {
    const state = item.fileViewer?.previewState ?? "ready";
    const text = item.fileViewer?.assetId
      ? `${item.fileViewer.assetId} (${state})`
  ...
  if (item.specialKind === "browser") {
    const title = item.browser?.title?.trim();
    const url = item.browser?.url?.trim() || "https://example.com";
    ...
    return {
      ...
      raw: {
        url,
        title,
        text: item.browser?.lastHtmlText ?? "",
      },
      text,
    };
  }
  ...
  const evaluateCustomItem = (itemId: string, stack = new Set<string>()): TelemetryItemDefinition | null => {
    ...
    if (stack.has(rawItem.id)) {
      const cyclicItem: TelemetryItemDefinition = {
        ...
        ai: rawItem.ai,
        fileManager: rawItem.fileManager,
        fileViewer: rawItem.fileViewer,
        browser: rawItem.browser,
