...
function buildTerminalPreview(item: TelemetryItemDefinition) {
  const liveOutput = item.terminal?.liveOutput?.trim() || "";
  return {
    shell: item.terminal?.shell ?? "bash",
    command: item.terminal?.command ?? "claude --resume",
    workingDirectory: item.terminal?.workingDirectory ?? "/workspace",
    streamOutput: item.terminal?.streamOutput ?? false,
    lines: (liveOutput ? liveOutput.split("\n") : ["ready"]).slice(-6),
  };
}
...
function getPluginPresentation(pluginId: PluginId, projectId: string, roleId: TeamPersonaId, data: WorkspaceDataSnapshot): WorkspaceNodePresentation {
  ...
  switch (pluginId) {
    case "analytics":
      return {
        ...
        signal: teamPersonaMap[roleId].interpretation,
      };
    case "revenue":
      return {
        icon: manifest.icon,
    ...
    case "agents": {
      ...
      return {
        icon: manifest.icon,
        accentClassName: manifest.accentClassName,
        status: agents.length ? "healthy" : "inactive",
        kindLabel: "Template",
        categoryLabel: "Agents",
        title: manifest.label,
        subtitle: manifest.capabilityLevel,
        headline: `${agents.length} agentes para ${teamPersonaMap[roleId].shortLabel}`,
        summary: agents[0]?.summary ?? manifest.description,
        metrics: [
          { label: "Ativos", value: agents.length.toString() },
          { label: "Modo", value: agents[0]?.mode ?? "coworker" },
...
export function getWorkspaceNodePresentation(
  ...
  if (binding.kind === "plugin") {
    return getPluginPresentation(binding.entityId as PluginId, projectId, roleId, data);
  }
  ...
  const agent = specialistAgentDefinitions.find((entry) => entry.id === binding.entityId);
  return {
    icon: BrainCircuit,
    accentClassName: "text-indigo-300",
    status: agent ? "healthy" : "inactive",
    kindLabel: "Agent",
    categoryLabel: "Autonomy",
    title: agent?.name ?? "Agent",
    subtitle: agent?.id ?? binding.entityId,
    headline: agent?.mode ?? "coworker",
    summary: agent?.summary ?? "Agente orientado por nos e sinais do projeto.",
    metrics: [
      { label: "Cargo", value: agent?.ownerRole ?? "-" },
      { label: "Scopos", value: agent?.scopes.length.toString() ?? "0" },
    ],
    tags: agent?.scopes ?? [],
  };
}
