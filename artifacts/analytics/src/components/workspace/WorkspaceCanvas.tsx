...
import { Activity, Blocks, BrainCircuit, FileText, Minus, Plus, ScanSearch, TableProperties, Terminal, Undo2 } from "lucide-react";
...
interface WorkspaceCanvasProps {
  ...
  onRunTerminalCommand?: (item: TelemetryItemDefinition, command: string) => Promise<void>;
...
export function WorkspaceCanvas({
  ...
  const [canvasMenuState, setCanvasMenuState] = useState<{ left: number; top: number } | null>(null);
  const [canvasSubmenu, setCanvasSubmenu] = useState<"create" | null>(null);
  const [terminalDraftByNodeId, setTerminalDraftByNodeId] = useState<Record<string, string>>({});
  const [runningTerminalNodeIds, setRunningTerminalNodeIds] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    if (!canvasMenuState) return;

    const handlePointerDown = (event: PointerEvent) => {
      const target = event.target as Node | null;
      if (target && canvasMenuRef.current?.contains(target)) return;
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      if (interaction.type === "move") {
        const heldLongEnough = performance.now() - interaction.armedAt >= 120;
        const moveActivated =
          interaction.activated || distance >= DRAG_THRESHOLD || (heldLongEnough && distance > CLICK_SLOP);
    ...
    const handlePointerUp = () => {
      if (interaction.type === "move") {
        ...
        const treatedAsDrag =
          ...
          interaction.travelDistance >= DRAG_THRESHOLD ||
          (heldLongEnough && interaction.travelDistance > CLICK_SLOP);
        const treatedAsClick = interaction.travelDistance <= CLICK_SLOP;
        ...
        if (treatedAsDrag) {
          ...
          setSelectedNodeIds([interaction.nodeId]);
  ...
  }, [interaction, onClearSelection, onMoveNode, onResizeNode, onSelectNode, onSetViewport, renderedNodes, selectedNodeId, view.viewport.zoom]);
  ...
  }, [onClearSelection, onRemoveNode, selectedNodeId, selectedNodeIds]);
  ...
  const handleCanvasPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (nodeElement?.dataset.workspaceNodeId) {
      ...
      if (connectingNodeId && connectingNodeId !== node.id) {
        onConnectNodes(connectingNodeId, node.id);
  ...
  const handleCanvasContextMenuCapture = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    setCanvasMenuState({
      left: Math.max(12, Math.min(event.clientX - rect.left, rect.width - menuWidth - 12)),
      top: Math.max(12, Math.min(event.clientY - rect.top, rect.height - menuHeight - 12)),
    });
    setCanvasSubmenu(null);
  };
  ...
  const handleZoom = (delta: number) => {
    onSetViewport({ zoom: clampZoom(viewport.zoom + delta) });
  };
  ...
  const handleTerminalDraftChange = (nodeId: string, nextCommand: string) => {
    setTerminalDraftByNodeId((current) => ({ ...current, [nodeId]: nextCommand }));
  };

  const handleTerminalRun = async (nodeId: string, item: TelemetryItemDefinition) => {
    if (!onRunTerminalCommand) return;

    const fallbackCommand = item.terminal?.command ?? "";
    const command = (terminalDraftByNodeId[nodeId] ?? fallbackCommand).trim();
    if (!command) return;

    setRunningTerminalNodeIds((current) => ({ ...current, [nodeId]: true }));
    try {
      await onRunTerminalCommand(item, command);
    } finally {
      setRunningTerminalNodeIds((current) => ({ ...current, [nodeId]: false }));
    }
  };
  ...
  return (
    ...
    >
      {!immersive ? (
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/70 bg-background/55 px-4 py-3 text-xs text-muted-foreground">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl" onClick={() => handleZoom(-0.1)}>
              <Minus className="h-4 w-4" />
            </Button>
            <span className="min-w-12 text-center font-mono text-[11px]">{Math.round(viewport.zoom * 100)}%</span>
            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl" onClick={() => handleZoom(0.1)}>
      ...
      >
        <div className="absolute inset-0 z-0" onContextMenuCapture={handleCanvasContextMenuCapture}>
          <div
            className="pointer-events-none absolute inset-0"
            style={{
              backgroundColor: immersive ? "#17191e" : "hsl(var(--card))",
              backgroundImage: immersive
                ? "radial-gradient(circle at 1.2px 1.2px, rgba(255,255,255,0.18) 1.2px, transparent 0), linear-gradient(180deg, rgba(0,0,0,0.48) 0%, rgba(0,0,0,0.18) 10%, rgba(0,0,0,0) 22%)"
                : "radial-gradient(circle at 1px 1px, hsl(var(--foreground) / 0.12) 1px, transparent 0)",
              backgroundSize: immersive ? `${GRID_SIZE}px ${GRID_SIZE}px, 100% 100%` : `${GRID_SIZE}px ${GRID_SIZE}px`,
              backgroundPosition: immersive ? `${viewport.x}px ${viewport.y}px, 0 0` : `${viewport.x}px ${viewport.y}px`,
            }}
          />
        </div>
        ...
        >
          <svg className="pointer-events-none absolute inset-0 h-full w-full overflow-visible">
            {edges.map((edge) => {
              ...
              const visual = getCanvasEdgeVisual(edge, isFocused);
              ...
              return (
                <g key={edge.id} opacity={selectedNode && !isFocused ? 0.22 : 1}>
                  <path
                    d={geometry.path}
                    fill="none"
                    stroke={visual.glow}
                  ...
                  <circle cx={geometry.start.x} cy={geometry.start.y} r={3.2} fill={visual.stroke} opacity={0.92} />
                  <circle
                    cx={geometry.end.x}
                    cy={geometry.end.y}
                    r={4.4}
                    ...
                    strokeWidth={1.7}
                  />
                  <g transform={`translate(${geometry.label.x - labelWidth / 2}, ${geometry.label.y - labelHeight / 2})`}>
                    <rect
                      width={labelWidth}
                      height={labelHeight}
                      rx={10}
          ...
          {renderedNodes.map((node) => {
            ...
            const presentationSignature = [
              ...
              presentation.metrics.map((metric) => `${metric.label}:${metric.value}`).join("|"),
            ].join("::");

            return (
              <ContextMenu key={`${node.id}:${presentationSignature}`}>
                <ContextMenuTrigger asChild>
                  ...
                  >
                    ...
                    >
                      <NodePortHandle side="left" active={isSelected || isConnectingSource} />
                      <NodePortHandle side="right" active={isSelected || isConnectingSource} />
                      <WorkspacePluginCard
                        ...
                        canvasMinimal
                        terminalRuntime={
                          presentation.displayVariant === "terminal" && item
                            ? {
                                command: terminalDraftByNodeId[node.id] ?? item.terminal?.command ?? "",
                                running: Boolean(runningTerminalNodeIds[node.id]),
                                onCommandChange: (nextCommand) => handleTerminalDraftChange(node.id, nextCommand),
                                onRun: () => void handleTerminalRun(node.id, item),
                              }
                            : undefined
                        }
                      />
                ...
                <ContextMenuContent className={cn(
                  ...
                  >
                    {connectingNodeId === node.id ? "Cancelar conexao" : "Conectar bloco"}
                  </ContextMenuItem>
              ...
              {renderedNodes.map((node) => (
                <div
                  ...
                  className={cn(
                    ...
                    connectingNodeId === node.id && "border-primary bg-primary/25",
                  )}
                  style={{
...
function NodePortHandle({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute top-1/2 z-20 h-4 w-4 -translate-y-1/2 rounded-full border shadow-[0_0_0_4px_rgba(11,14,18,0.9)]",
...
function clampZoom(zoom: number) {
  return Math.max(0.5, Math.min(1.6, Number(zoom.toFixed(2))));
}
