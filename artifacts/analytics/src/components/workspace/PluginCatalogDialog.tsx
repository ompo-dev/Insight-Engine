...
export function PluginCatalogDialog({
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (onlyRecommended && !item.recommendedFor.includes(roleId))
        return false;
  ...
  }, [category, items, onlyRecommended, roleId, search]);
        ...
        <div className="border-b border-border/70 px-6 py-4">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                Recomendados para {teamPersonaMap[roleId].shortLabel}
              </Button>
            </div>
          </div>
        ...
        <ScrollArea className="h-[64vh]">
          <div className="grid gap-4 px-6 py-6 lg:grid-cols-2">
            {visibleItems.map((item) => {
              ...
              const recommended = item.recommendedFor.includes(roleId);
