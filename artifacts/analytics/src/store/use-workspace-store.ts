...
function getScopeKey(projectId: string, roleId: WorkspaceRoleId) {
...
function sanitizeWorkspaceDefinition(definition: LegacyWorkspaceDefinition): WorkspaceDefinition {
  ...
  const tabs = migratedTabs.length ? migratedTabs : createDefaultWorkspaceDefinition(definition.projectId, definition.roleId).tabs;
  ...
  persist(
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        ...
        const definitionsByScope = Object.fromEntries(
          Object.entries(state?.definitionsByScope ?? {}).map(([scopeKey, definition]) => [
            scopeKey,
            sanitizeWorkspaceDefinition(definition as LegacyWorkspaceDefinition),
          ]),
        );
